...
EOF
		...
		if [ -f "${i}-packages-nr" ]; then
			...
			if [ -n "$PACKAGES" ]; then
				on_chroot << EOF
apt-fast -o APT::Acquire::Retries=10 install --no-install-recommends -y $PACKAGES
EOF
				if [ "${USE_QCOW2}" = "1" ]; then
					on_chroot << EOF
apt-fast clean
EOF
		...
		if [ -f "${i}-packages" ]; then
			...
			if [ -n "$PACKAGES" ]; then
				on_chroot << EOF
apt-fast -o APT::Acquire::Retries=10 install -y $PACKAGES
EOF
				if [ "${USE_QCOW2}" = "1" ]; then
					on_chroot << EOF
apt-fast clean
...
export PI_GEN_REPO=${PI_GEN_REPO:-https://github.com/p4edge/image}
...
export GIT_HASH=${GIT_HASH:-"$(git rev-parse --short=8 HEAD)"}

export USE_QEMU="${USE_QEMU:-0}"
export IMG_DATE="${IMG_DATE:-"$(date +%Y-%m-%d)"}"
export IMG_FILENAME="${IMG_FILENAME:-"${IMG_NAME}-${IMG_DATE}-${GIT_HASH}"}"
...
export TIMEZONE_DEFAULT="${TIMEZONE_DEFAULT:-Europe/Budapest}"
